...
for(i in 1:dim(test_data)[1]){
  pred=predict(model[[class[i]]],newdata=test_data[i,4:12],interval="prediction", level = 0.9)
...
lm=lm(log(gross)~screens+weight_budget+weight_cast1+weight_cast2+weight_cast3+weight_director+weight_genres+weight_month+weight_superstar,data=train_data)
...
pred=predict(lm,newdata=test_data[,4:12],interval="prediction", level = 0.9)#原本4:11
